...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ---------- Interactive before/after compare slider (legacy, kept for compatibility) ----------
  ...
  // ---------- Evolution stepper (growth story: 5 000 → 8 000 → 10 000) ----------
  document.querySelectorAll('.evolution-card').forEach(card => {
    const steps = Array.from(card.querySelectorAll('.evolution-step'));
    const shots = Array.from(card.querySelectorAll('.evolution-shot'));
    const progress = card.querySelector('.evolution-progress');
    const countEl = card.querySelector('.evolution-count');
    if (!steps.length || !shots.length || !countEl) return;

    let currentValue = parseInt((countEl.textContent || '0').replace(/\s/g, ''), 10) || 0;
    let animId = null;
    ...
    function animateCountTo(target) {
      ...
      function tick(now) {
        ...
        countEl.textContent = value.toLocaleString('fr-FR');
        if (p < 1) { animId = requestAnimationFrame(tick); }
        else { currentValue = target; countEl.textContent = target.toLocaleString('fr-FR'); }
      }
      animId = requestAnimationFrame(tick);
    }

    function activate(stepValue, opts = {}) {
      const idx = steps.findIndex(s => s.dataset.step === String(stepValue));
      if (idx === -1) return;
      ...
      shots.forEach(shot => {
        shot.classList.toggle('active', shot.dataset.shot === String(stepValue));
      });
      if (progress) {
        const pct = steps.length > 1 ? (idx / (steps.length - 1)) * 100 : 0;
        progress.style.width = pct + '%';
      }
      if (!opts.skipCount) animateCountTo(parseInt(stepValue, 10));
    }
    ...
    // Initial state: show the final milestone immediately, no count-up
    const lastStep = steps[steps.length - 1];
    if (lastStep) {
      activate(lastStep.dataset.step, { skipCount: true });
      currentValue = parseInt(lastStep.dataset.step, 10);
      countEl.textContent = currentValue.toLocaleString('fr-FR');
    }

    // Auto-play the growth story once, the first time the card scrolls into view
    let played = false;
    if ('IntersectionObserver' in window) {
      const playIO = new IntersectionObserver((entries, obs) => {
        entries.forEach(entry => {
          ...
          order.forEach((val, i) => {
            setTimeout(() => activate(val), 400 + i * 800);
          });
        });
      }, { threshold: 0.5 });
      playIO.observe(card);
